 ...
 */
export const ReasoningSelect = ({ value, onChange, disabled, selectedModel, currentProvider, piThinkingLevels = [] }: ReasoningSelectProps) => {
  ...
  const availableLevels = REASONING_LEVELS.filter(level => {
    if (currentProvider === 'pi' && piThinkingLevels.length > 0) {
      return piThinkingLevels.includes(level.id);
    }

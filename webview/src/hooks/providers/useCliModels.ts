...
function normalizeModels(raw: unknown): ModelInfo[] {
  ...
  for (const item of raw) {
    ...
    // Pi can expose the same model id through several providers. The catalog
    // identity is therefore provider + id, not id alone.
    const identity = provider ? `${provider}::${id}` : id;
    ...
    out.push({ id: identity, label, description, provider: provider || undefined });
  }
  return out;
}
